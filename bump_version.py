...
def main() -> None:
    ...
    # No calendar date in this heading, on purpose - every real entry in
    # this CHANGELOG (see the ones above this one) is dated only by its
    # position in the file, never a literal date, and this generated stub
    # is meant to be replaced with a real description before committing
    # anyway (see every other repo's own build.sh/bump_version.py in the
    # ecosystem for the same convention).
